...
import { readableStreamToString, TerminalError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TerminalError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TerminalError(e, e.response?.data)
                }
                if (innerError instanceof TerminalError) {
                    // Got "TerminalError" from the above try block
                    throw innerError;
                }
